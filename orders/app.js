...
// *Mayıs 1998 yılında verilen siparişleri listeleyin.
// *Mayıs 1998 yılında verilen siparişlerin toplam tutarı nedir?
// *Bugüne kadar en düşük siparişi veren müşteri (bad customer)?
...
//      if (new Date(order.orderDate).getFullYear() === 1998) {
//          console.log(new Date(order.orderDate).getMonth(4), order)
//      }
//  });
...
// *Mayıs 1998 yılında verilen siparişlerin toplam tutarı nedir?
 orders.forEach(order => {
     if(order.details){
        if (new Date(order.orderDate).getFullYear() === 1998 && new Date(order.orderDate).getMonth() === 4) {
...
console.log(customerTotalPrice.length);

orders.forEach((order) =>{
    if(order.details){
        let object = {id: order.customerId, totalPrice: 0};
       ...
       object.totalPrice = -totalOrderPrice;
       const result = customerTotalPrice.length > 0 ? customerTotalPrice.some(q => q.id === order.customerId) : null
         if (result) {
             ...
             customerTotalPrice.push(object)
         }
    }
});
badCustomer = customerTotalPrice.sort((a,b) => a.totalPrice-b.totalPrice)[0]

console.log(customerTotalPrice);
console.log(badCustomer);